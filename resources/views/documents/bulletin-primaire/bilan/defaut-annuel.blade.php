<!-- Tableau de bilan -->
<div style="width: 100%;">
    <table style="width: 100%; border-collapse: collapse; margin-left: 0; page-break-inside: avoid">
        <tr style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}};">
            <td colspan="4" style="width: 80px;border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    <strong>{{ count($sequences) > 1 ? __('bulletin_primaire.res_analys_trim') : __('bulletin_primaire.res_analys') }}</strong>
                </p>
            </td>
            <td colspan="2" style="border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    <strong>{{ __('bul_mat.visa_dir') }}</strong>
                </p>
            </td>
            <td colspan="2" style="border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    <strong>{{ __('bul_mat.visa_ens') }}</strong>
                </p>
            </td>
            <td colspan="2" style="border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    <strong>{{ __('bul_mat.visa_par') }}</strong>
                </p>
            </td>
        </tr>

        <tr>
            @php
                $moyenne = $evaluation['moyenneGenerale'] ?? null;
                $moyenneTrimestre = ($evaluation['totalNoteObtenus'] !== null && $evaluation['totalNoteMax'] != null) ? ($evaluation['totalNoteObtenus'] / $evaluation['totalNoteMax']) * 20 : null;

                $appreciation = getAppreciationGradeAndColor($moyenne, 20);

                if(count($donneesClasse['moyennesObtenues']) > 0)
                {
                    $bestMoyenne = $donneesClasse['moyennesObtenues'][0];
                    $gradeBestMoyenne = getAppreciationGradeAndColor($bestMoyenne, 20);

                    $worstMoyenne = end($donneesClasse['moyennesObtenues']);
                    $gradeWorstMoyenne = getAppreciationGradeAndColor($worstMoyenne, 20);
                }
                else{
                    $bestMoyenne = null;
                    $gradeBestMoyenne = getAppreciationGradeAndColor(0, 20);

                    $worstMoyenne = null;
                    $gradeWorstMoyenne = getAppreciationGradeAndColor(0, 20);
                }
            @endphp

            <td colspan="2" style="width: 27px;border: 1pt solid #808080">
                <p class="s10" style="font-weight:bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    @php $appreciation = getAppreciationGradeAndColor($moyenne, 20); @endphp
                    TOTAL:
                    <span style="color: #{{$legend_of_grade[$appreciation[1]]}}">
                        @if(count($trimestres) > 1)
                            {{ number_format_if_float($evaluation['totalNoteObtenus'] / count($trimestres), 2) }}
                        @else
                            {{ number_format_if_float($evaluation['totalNoteObtenus'], 1) }}
                        @endif
                        /{{ number_format_if_float($evaluation['totalNoteMax'] / count($trimestres)) }}
                    </span>
                </p>
            </td>

            <td style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}}; width: 27px; border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    {{ __('bulletin_primaire.high_av') }}
                </p>
            </td>

            <td style="width: 27px; border: 1pt solid #808080" >
                <p class="s10" style="font-weight:bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$gradeBestMoyenne[1]]}}">
                    @if(count($sequences) > 1)
                        {{ number_format_if_float($bestMoyenne, 1) }}/20
                    @else
                        {{ number_format_if_float($bestMoyenne, 2) }}/20
                    @endif
                </p>
            </td>
            <td colspan="2" rowspan="4" style="width: 20%;border: 1pt solid #808080; text-align: center; vertical-align: middle">
                @if(file_exists(public_path('public/profil/'. $ecole->principal->signature)))
                    <img
                        src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path('public/profil/'. $ecole->principal->signature))) }}"
                        alt="Signature"
                        style="height: 60px; object-fit: contain;"
                    >
                @endif
            </td>
            <td colspan="2" rowspan="4" style="width: 20%;border: 1pt solid #808080; text-align: center; vertical-align: middle;">
                @if(!empty($infosClasse['signatureEnseignant']) && file_exists(public_path('public/profil/' . $infosClasse['signatureEnseignant'])))
                    <img
                        src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path('public/profil/'. $infosClasse['signatureEnseignant']))) }}"
                        alt="Signature"
                        style="height: 60px; object-fit: contain;"
                    >
                @endif
            </td>

            <td colspan="2" rowspan="4" style="width: 20%;border: 1pt solid #808080"></td>
        </tr>

        <tr>
            @if(count($trimestres) > 1)
                <td colspan="2" rowspan="1" style="border: 1pt solid #808080; color: white; background-color: #{{$evaluation['isEvalue']? $legend_of_grade[$appreciation[1]] : '000000'}}; white-space: nowrap; padding: 2pt;">
                    <p class="s10" style="text-indent: 0pt; line-height: 10pt; text-align: center; color: white; margin: 0;">
                        <strong>{{ __('bulletin_primaire.avg') }}</strong>:
                        <span style="font-weight: bold; font-size: 14px;">
                            {{ number_format_if_float($moyenne, 2) }}
                        </span>
                        <strong>/20</strong>
                    </p>
                </td>
            @else
                <td colspan="2" rowspan="2" style="border: 1pt solid #808080; color: white; background-color: #{{$evaluation['isEvalue']? $legend_of_grade[$appreciation[1]] : '000000'}}; white-space: nowrap; padding: 2pt;">
                    <p class="s10" style="text-indent: 0pt; line-height: 10pt; text-align: center; color: white; margin: 0;">
                        <strong>{{ __('bulletin_primaire.avg') }}</strong>:
                        <span style="font-weight: bold; font-size: 20px;">
                            @if(count($sequences) > 1)
                                {{ number_format_if_float($moyenneTrimestre, 10) }}
                            @else
                                {{ number_format_if_float($moyenneTrimestre, 10) }}
                            @endif
                        </span>
                        <strong>/20</strong>
                    </p>
                </td>
            @endif

            <td style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}}; width: 27px;border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    {{ __('bulletin_primaire.low_avg') }}
                </p>
            </td>

            <td style="width: 20%;border: 1pt solid #808080" >
                <p class="s10" style="font-weight:bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$gradeWorstMoyenne[1]]}}">
                    @if(count($sequences) > 1)
                        {{ number_format_if_float($worstMoyenne, 1) }}/20
                    @else
                        {{ number_format_if_float($worstMoyenne, 2) }}/20
                    @endif
                </p>
            </td>
        </tr>

        <tr>
            <td colspan="2" style="width: 27px;border: 1pt solid #808080; ">
                <p class="s10" style="font-size: 12px; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$appreciation[1]]}}">
                    <strong>
                        {{ __('bulletin_primaire.rank') }}:
                        {!! $evaluation['isEvalue']
                                ? getStudentRank((array_search($moyenne, $donneesClasse['moyennesObtenues']) + 1))
                                : '-'
                        !!}
                    </strong>
                </p>
            </td>

            <td style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}}; width: 27px;border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    {{ __('bulletin_primaire.class_avg') }}
                </p>
            </td>
            <td style="width: 20%;border: 1pt solid #808080" >
                @php
                    $classAvgColor = getAppreciationGradeAndColor($donneesClasse['moyenneGenerale'], 20);
                @endphp
                <p class="s10" style="font-weight:bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$classAvgColor[1]]}}">
                    @if(count($sequences) > 1)
                        {{ number_format_if_float($donneesClasse['moyenneGenerale'], 1) }}/20
                    @else
                        {{ number_format_if_float($donneesClasse['moyenneGenerale'], 2) }}/20
                    @endif
                </p>
            </td>
        </tr>

        <tr>
            @if(count($trimestres) > 1)
                <td colspan="2" style="width: 27px;border: 1pt solid #808080;">
                    <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$appreciation[1]]}}">
                        <strong style="font-size: 14px;">{{ $decisionAnnuelle ?? '-' }}</strong>
                    </p>
                </td>
            @endif

            <td style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}}; width: 59px;border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    % {{ __('bulletin_primaire.success') }}
                </p>
            </td>
            <td style="width: 20%;border: 1pt solid #808080" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                    @php $classSuccessPercentageColor = getAppreciationGradeAndColor($donneesClasse['PourcentageReussite'], 100); @endphp
                    <strong style="color: #{{$legend_of_grade[$classSuccessPercentageColor[1]]}}">
                        {{ number_format_if_float($donneesClasse['PourcentageReussite'], 2) }}%
                    </strong>
                </p>
            </td>
        </tr>
    </table>
</div>
