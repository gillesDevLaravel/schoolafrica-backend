...
<table style="text-align: center;  width: 100%; margin-bottom: 8px">
    <tr>
        <td style="color:#{{$code_couleurs[0]}}; text-align: center; font-size:20px; "><strong>{{ strtoupper($school->name) }}</strong></td>
    </tr>
</table>
...
<table width="90%" style="margin-left: 5%;">
    <tr>
        <td style="font-size: 17px;text-align:center; background-color:green;padding:5px; color:white;display:flex;" >{{ __('bul_mat.bul_notes') }}</td>
        <td style="font-size: 17px;text-align:center; background-color:red;padding:5px; color:white;">{{ __('bul_mat.ensgn_franc') . " ". $assessmentType->name }}</td>
        <td style="font-size: 17px;text-align:center; background-color:rgb(239, 239, 49);padding:5px; color:rgb(14, 14, 14);">2024/2025</td>
    </tr>
</table>
...
<table width="100%" style="border-collapse: collapse; margin-left: 0;">
    <tr>
        ...
        <td colspan="2">
            <p style="font-size:16px; margin-left: 1pt; color: green"><strong>{{ $user->name }}</strong></p>
        </td>
        <td colspan="2">
            <p style="font-size:16px; margin-left: 1pt; color: green"><strong>{{ $classe->name }}</strong></p>
        </td>
    </tr>
    <tr>
        <td >
            <p style="font-size:16px; margin-left: 1pt">
    ...
    <table width="100%" style="border-collapse: collapse; margin-left: 0;">
        <tr style="background-color: #{{couleurEnteteTableauBulletinJunior()['bg']}};">
            <td class="td_border" style="width: 35%">
                <p class="s10" style="font-size:16px; font-weight: bold; padding: 2pt; text-align: center; color: #{{couleurEnteteTableauBulletinJunior()['txt']}}; ">
                    {{ __('bul_mat.dom') }}
                </p>
            </td>
            <td class="td_border" style="width: 37%">
                <p class="s10" style="font-size:16px; font-weight: bold; padding: 2pt; text-align: center; color: #{{couleurEnteteTableauBulletinJunior()['txt']}}; ">
                    {{ __('bul_mat.acti') }}
                </p>
            </td>
            <td class="td_border" style="width: 5%;" >
                <p class="s10" style="font-size:16px; font-weight: bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{couleurEnteteTableauBulletinJunior()['txt']}}">
                    {{ __('bul_mat.note') }}
                </p>
            </td>
            <td class="td_border" style="width: 10%;" >
                <p class="s10" style="font-size:16px; font-weight: bold; padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{couleurEnteteTableauBulletinJunior()['txt']}}">
                    {{ __('bul_mat.appr') }}
                </p>
            </td>
            <td class="td_border" style="width: 6%;" >
                <p class="s10" style="font-size:16px; font-weight: bold; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{couleurEnteteTableauBulletinJunior()['txt']}}">
                    {{ __('bul_mat.remq') }}
{{--                    RMQ--}}
                </p>
            </td>
        ...
        @foreach($user->matterGroup as $matterGroup)
            ...
            <tr style="height: 16pt;">
                <td style="width: 100pt; border: 1pt solid #808080; vertical-align: middle;">
                    <p class="s13" style="font-size:17px;padding: 1px; text-align: center; margin: 0; font-weight: bold">{{ $assessment->nameMatter }}</p>
                </td>
                <td style="padding-left:0pt!important; width: 100pt;border: 1pt solid #808080;vertical-align:middle;">
                    @foreach($assessment->typeEvaluation as $key => $typeEvaluation)
                        <p class="s14" style="font-size:15px;vertical-align:middle; text-align:center;@if($key != count($assessment->typeEvaluation)-1)border-bottom: 1pt solid #808080;@endif height: 25px;">
...
{{--                                        {{ $note }} | {{ $note_max }}--}}
                                        @if(!is_null($note))
                                        <img style="text-align: center; width: 25px; height: 25px;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/$grade_img"))) }}">
                                        @else
                                            <img style="text-align: center; width: 25px; height: 25px;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/appreciation0.png"))) }}">
                                        @endif
                                    </p>
                                @endif
                            @endforeach
                ...
                <td style="padding-left:0pt!important; text-align:center; width: 10pt;border: 1pt solid #808080">
                    @php
                        $note_assessment = $assessment->{"total_note_assessment".mb_substr($assessmentType->name, -1)}; //$assessment->total_note_assessment1;
                        $grade_img_assessment = getAppreciationSticker($note_assessment, $assessment->notemax); @endphp
                    @if($note_assessment>0)
                    <img class="appreciation_img" style="width: 80px; height: 80px" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/$grade_img_assessment"))) }}">
                    @else
                        <img style="text-align: center; width: 25px; height: 25px;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/appreciation0.png"))) }}">
                    @endif
{{--                    {{ $note_assessment }} | {{ $assessment->notemax }}--}}
                </td>
                <td style="padding-left:0pt!important; text-align:center; width: 5pt;border: 1pt solid #808080">
                </td>
            </tr>
        @endforeach
        @endforeach
...
{{--            <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;"></p>--}}
    ...
    <tr>
        <td style="width: 20%;" class="td_border" >
            <p class="s10" style="font-size:15px;padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: #{{$legend_of_grade[$moyenneColor[1]]}}"> {{ $appreciation_txt }}</p>
        </td>
    </tr>
{{--    <tr>--}}
{{--        <td style="width: 20%;" class="td_border" >--}}
...
{{--                {{ __('bul_mat.decision') }}:--}}
{{--                @if($moyenne >= 10)--}}
{{--                <strong style="color: #{{$legend_of_grade[$moyenneColor[1]]}}">{{ __('bul_mat.admis') }}</strong>--}}
{{--                @else--}}
{{--                <strong style="color: #{{$legend_of_grade[$moyenneColor[1]]}}">{{ __('bul_mat.echoue') }}</strong>--}}
{{--                @endif--}}
{{--            </p>--}}
{{--        </td>--}}
{{--    </tr>--}}
