    ...
    <tr>
        ...
        <td colspan="2" style="width: 200px">
            <p style="margin-left: 1pt">
                ...
                {{__('bulletin_primaire.effectif')}}: <strong>{{ $effectifClasse }}</strong> <br>
...
>
    <table width="100%" style="border-collapse: collapse; margin-left: 0;">
        <tr style="background-color: #{{$code_couleurs[0]}};">
            ...
            <td class="td_border" style="width: 50pt;" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: white">
                    <strong>{{ __('bul_mat.appr') }}</strong>
                </p>
            </td>
            <td class="td_border" style="width: 50pt;" >
                <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center; color: white">
                    <strong>{{ __('bul_mat.remq') }}</strong>
                </p>
            </td>
        </tr>

        @foreach($user->trimestre[0]->assessmentType[0]->matterGroup as $keyM => $matterGroup)
            @php
                $totalTrimMatterGroup[] = 0;
            @endphp
            <tr>
                <td class="td_border">
                    <p class="s10" style="text-align: center;">
                        <strong>{{ $matterGroup->name }}</strong>
                    </p>
                </td>
                <td class="td_border">
                    @foreach($matterGroup->assessment as $key => $assessment)
                        <p class="s14" style="height: 25px; padding: 1px; text-align:center;@if($key != count($matterGroup->assessment)-1)border-bottom: 1px solid #808080;@endif">
                            {{ $assessment->nameMatter ?? "-" }}
                        </p>
                    @endforeach
                </td>

                @foreach($trimestre->assessmentTypes as $keySeq => $seq)
                    @php
                        // On va ajouter des opérations ici qui seront utiles plus bas même pour d'autres 'td'

                        // On calcule la moyenne de l'appréciation par groupe de matière sur le trimestre
                        $totalTrimMatterGroup = array();
                        $nbreSequences = 0; // pour la division afin d'avoir la moyenne adéquate
                    @endphp
                <td class="td_border">
                    @php $totalTrimMatterGroup[] = 0; @endphp
                    @foreach($user->trimestre[0]->assessmentType[$keySeq]->matterGroup as $keyMG => $tmpMatterGroup)
{{--                        @php $totalTrimMatterGroup[$keyMG] += $tmpMatterGroup->MoyenneMatterGroup; if($tmpMatterGroup->MoyenneMatterGroup>0) $nbreSequences++; @endphp--}}

                        @if($matterGroup->id == $tmpMatterGroup->id)
                            @foreach($tmpMatterGroup->assessment as $key => $assessment)
                                @php
                                ...
                                @endphp
                                <p class="s14" style="text-align:center;@if($key != count($tmpMatterGroup->assessment)-1)border-bottom: 1px solid #808080;@endif">
                                    @if(!is_null($noteToCheck))
                                        <img class="appreciation_img" style="text-align: center;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/$grade_img"))) }}">
                                    @else
                                        <img class="appreciation_img" style="text-align: center;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/appreciation1.png"))) }}">
                                    @endif
                                </p>
                            @endforeach
                        @endif
...
{{--                        <p class="s14" style="text-align:center;@if($keyA != count($matterGroup->assessment)-1)border-bottom: 1px solid #808080;@endif">--}}
{{--                            {{ @$user->trimestre[0]->assessmentType[$keySeq]->matterGroup[$keyM]->assessment[$keyA]->ratings->value }}--}}
{{--                        </p>--}}
{{--                    @endforeach--}}
{{--                    @php $sum_ratings = 0 @endphp--}}
{{--                    @foreach($user->trimestre[0]->assessmentType[$key]->matterGroup as $matterGroup)--}}
{{--                        @foreach($matterGroup->assessment as $keyA => $assessment)--}}
{{--                            @php--}}
{{--                                $notemaxToCkech = $assessment->notemax;--}}
{{--                                $noteToCheck = @$assessment->ratings->value;--}}
...
{{--                            <p style="line-height: 10pt; @if($keyA != count($matterGroup->assessment)-1)border-bottom: 1px solid #808080;@endif">--}}
{{--                                yoo {{$keyA}} / {{count($matterGroup->assessment)-1}}--}}
{{--                                {{count($matterGroup->assessment)}}--}}
{{--                                @if(!is_null($noteToCheck))--}}
{{--                                    <img class="appreciation_img" style="text-align: center;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/$grade_img"))) }}">--}}
{{--                                @else--}}
{{--                                    <img class="appreciation_img" style="text-align: center;" src="data:image/png;base64,{{ @base64_encode(file_get_contents(public_path("/public/profil/appreciation1.png"))) }}">--}}
        ...
        <td style="width: 20%;" class="td_border" >
            <p class="s10" style="padding: 2pt; text-indent: 0pt; line-height: 10pt; text-align: center;">
                @php
                    ...
                    if($moyenne < 1) { $appreciation_img = "appreciation1.png"; $appreciation_txt = __('bulletin_primaire.appr_nye_txt'); }
                    else if($moyenne < 2) { $appreciation_img = "appreciation2.png"; $appreciation_txt = __('bulletin_primaire.appr_ae_txt'); }
                    else if($moyenne < 3) { $appreciation_img = "appreciation3.png"; $appreciation_txt = __('bulletin_primaire.appr_me_txt'); }
